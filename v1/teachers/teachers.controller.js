...
const getAllTeacher = async (req, res) => {
  try {
    ...
    const teachers = await pool.query(`SELECT * FROM teachers WHERE LOWER(first_name) LIKE LOWER($1)`,[`%${name || ''}%`]);
...
const createTeacher = async (req, res) => {
  try {
    ...
    await pool.query(`INSERT INTO teachers (first_name,second_name) VALUES ($1,$2)`, [
      first_name,
      second_name,
    ]);
